...
func TestInstallChartUsesCorrectFlagsForHelmVersion(t *testing.T) {
	testcases := []struct {
		...
		{
			name:        "helm v4.0 enables server-side apply with conflict forcing and translates --atomic",
			helmVersion: "4.0.0",
			flags:       []string{"--atomic"},
			expectedArgs: []string{
				"helm",
		...
		{
			name:        "helm v4.1 enables server-side apply with conflict forcing without extra flags",
			helmVersion: "4.1.1",
			flags:       nil,
			expectedArgs: []string{
				"helm",
